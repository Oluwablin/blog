...
function populateStates(selectedCountryIndex, stateElement, stateValue=null) {
  ...
  var data_state = stateElement.dataset.state;
    ...
    for (var i = 0; i < state_arr.length; i++) {
       console.log(state_arr[i], 'state arr');
       console.log(stateValue, 'state val');
...
function populateCountries(countryElement, stateElement) {
  ...
  console.log(country_value, "contry elem");
  ...
  if (data_country == "") {
    ...
    if (stateElement != "" && stateElement != null) {
      populateStates(countryElement.selectedIndex, stateElement);
    }
  }
  ...
  if (stateElement != "" && stateElement != null) {
    countryElement.addEventListener("change", function () {
      ...
      stateElement.length = 0;
      console.log(countryElement, 'Country Elem');
      console.log(stateElement, 'State Elem');
      populateStates(countryElement.selectedIndex, stateElement);
    });
  }
}
// to intialize and load countries
function loadCountries(state='Lagos') {
  ...
  if (location_groups != "" && location_groups != null) {
    for (var i = 0; i < location_groups.length; i++) {
      ...
      populateStates(169, state_el, state);
    }
  }
}
